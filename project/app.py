...
# ---------------------------
    ...
    import random
    if "cart_random_item" not in st.session_state:
        st.session_state.cart_random_item = random.choice(menu)

    if st.button("Get Random Food", key="cart_rand_btn"):
        st.session_state.cart_random_item = random.choice(menu)
        st.rerun()

    item = st.session_state.cart_random_item
    st.info(f"🍽 Recommended: **{item['name']}** — RM {item['price']}")

    if st.button("➕ Add to Cart", key=f"cart_rand_add_{item['id']}"):
        cart_module.add_item(
            cart,
            {
                "id": item["id"],
                "name": item["name"],
    ...
    # -----------------------------
    # 🥤 SPECIAL DRINK RECOMMENDATION
    # -----------------------------
    st.subheader("🥤 Special Drink Recommendation")

    drinks = [m for m in menu if m["category"] == "Drink"]

    if "cart_drink_item" not in st.session_state:
        st.session_state.cart_drink_item = random.choice(drinks)

    if st.button("Recommend a Drink", key="cart_drink_btn"):
        st.session_state.cart_drink_item = random.choice(drinks)
        st.rerun()

    ditem = st.session_state.cart_drink_item
    st.info(f"🥤 Drink: **{ditem['name']}** — RM {ditem['price']}")

    if st.button("➕ Add Drink to Cart", key=f"cart_drink_add_{ditem['id']}"):
        cart_module.add_item(
            cart,
            {
                "id": ditem["id"],
